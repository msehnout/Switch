...
main = do
  ...
  printLog Tg.control $ "Starting switch on port: " ++ show 4242
...
acceptLoop socket channels = do
  ...
  --show debug output
  printLog Tg.control $ "New user connected from: " ++ hostname ++ ":" ++ show port
      ...
      Just msg -> do
        ...
        case mAddr of
          Just addr -> do -- send control message do switch thread
            printLog Tg.control $ "Obtained address request from client: " ++ show addr
            return $ Just addr
    ...
    Right clientTuple -> do
        ...
        if msgType == RequestAddr
           then do
               ...
               case maybeChannel of
                 ...
                 Nothing -> do
                   ...
                   printLog Tg.control $ "Added new client with address: " ++ show addr ++ "\
                   \. Clients:" ++ dbgShowClients (newClientTuple:clients) ""
                   hPutStrLn handle respondAccepted
                   switch channels (newClientTuple:clients)
                 --Don't accept new client.
                 Just _ -> do
                   printLog Tg.control $ "Attempt to use duplicate address. Connection closed."
           ...
           -- Timeout of some user exceeded or EOF reached. The user is going to be removed.
               ...
               case maybeChannel of
                 Nothing -> do
                     printLog Tg.err $ "Internal error occured. Attempt to delete non-existing address"
                     switch channels clients
                 Just chan -> do
                     let newClients = addr `removeFrom` clients
                     printLog Tg.control $ "Client " ++ show addr ++ " removed. Clients: " ++ dbgShowClients newClients ""
                     hClose handle
                     switch channels newClients
                     where removeFrom a = toList.(delete a).fromList
         -- Print connected users
   where dbgShowClients [] str = str ++ ";"
         dbgShowClients (x:xs) str = do
            let (addr,_) = x
                newStr = str ++ "("++show addr++"),"
            dbgShowClients xs newStr
...
readFromClient addr handle controlChannel channel killSignal = do
  ...
  case maybeInput of
    ...
    Nothing -> do
      printLog Tg.control $ "Timeout for client " ++ show addr
    ...
    Just maybeMsg ->
      ...
      case maybeMsg of
        Nothing -> do
          printLog Tg.control $ "Reached EOF of client " ++ show addr
          killClient
...
writeToClient addr handle channel killSignal = do
  ...
  case input of
    ...
    Right signal -> do
        if signal == "kill"
           then do
             printLog Tg.control "Client write thread killed"
             hClose handle
           else printLog Tg.err "Client write thread obtained unknown signal"
